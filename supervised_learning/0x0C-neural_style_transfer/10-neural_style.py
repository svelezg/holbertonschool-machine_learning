...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1, var=10):
        ...
        err = 'var must be a non-negative number'
        if not isinstance(beta, (int, float)) or var < 0:
            raise TypeError(err)
    ...
    def total_cost(self, generated_image):
        ...
        J_var = self.var * (self.style_cost(generated_output[:-1]) +
                            self.content_cost(generated_output[-1]))
    ...
    def compute_grads(self, generated_image):
        ...
        grad = tape.gradient(J_var, generated_image)
    ...
    def generate_image(self, iterations=1000, step=None,
        ...
        for i in range(iterations):
            # compute gradient
            grad, J_total, J_content, J_style, J_var = \
